---
layout: single
title: 타입스크립트에서 타입을 알 수 없는 외부 데이터 안전하게 처리하기
date: 2023-03-06 21:00:00 +0900
categories: software
---

타입스크립트로 개발을 할 때에, 네트워크 요청 등 방식을 통해 타입을 알 수 없는 외부 데이터를 가져오게 되는 경우가 있습니다. 이 글에서는 우선 타입스크립트에서 타입을 알 수 없는 데이터를 다루는 가장 흔한 방법들에 대해서 알아보고, Zod라는 라이브러리를 사용해 다른 방식들의 단점을 보완해 데이터를 안전하게 다룰 수 있는 방법에 대해서 소개해드리고자 합니다.
...
단순히 가장 빠르게 개발하는 것에 초점을 맞추자면... 그냥 별도의 타입 정보를 추가하지 않는 것도 방법이 될 수 있습니다. 이때 `getUserData`는 `any` 타입을 반환하게 됩니다.
...
이렇게 **타입을 알 수 없는 데이터에 대해 `any` 타입을 사용하는 것은 최대한 피해야 할 방법**입니다. 타입스크립트에서 `any`는 타입을 지정하지 않은 것과 다름이 없습니다. `any` 타입의 데이터를 참조할려고 하면, 타입스크립트가 제공하는 정적 분석, 코드 자동 완성 등의 기능의 도움을 전혀 받을 수 없게 됩니다.
...
![recommended-requiring-type-checking 설정 포함 후 any를 사용할 때 에러가 나는 모습](/assets/images/2023-03-06/1-linter.png)
_ESLint에서 [recommended-requiring-type-checking](https://typescript-eslint.io/linting/configs/#recommended-requiring-type-checking) 설정 포함 후 `any` 타입을 사용할 때 에러가 나는 모습_
{: style="color:gray; font-size: 80%; text-align: center;"}

## 2. Type Narrowing

타입스크립트의 Type Narrowing 기능은 타입을 알 수 없는 데이터의 타입을 확인할 수 있는 방법 중 하나입니다. 가장 흔히 쓰이는 방법은 `typeof` 연산자와 `if`문을 같이 활용하는 것입니다. 이를 활용하면 외부에서 들어온 `data`의 타입이 기대와 동일한지를 런타임에 검증할 수 있고, 예기치 못한 타입이 내려왔을 때에도 적절히 대응할 수 있습니다.
...
이렇게 Type Narrowing을 통해 데이터의 타입을 검증할 수 있지만, 이 방식에도 여러 단점이 있습니다.
...
하지만 이 방식에도 치명적인 단점이 있는데요, 바로 **데이터가 실제로 지정된 타입과 일치하는지 런타임에 전혀 검증이 되지 않는다**는 것입니다. 외부 데이터의 타입이 기대했던 것과 다르다고 해도, `as`키워드로 Type Assertion을 하는 시점에는 에러가 발생하지 않고, 반환된 데이터를 참조할 때서야 `undefined`가 나와 이로 인한 오류가 발생하게 됩니다.

이는 타입스크립트의 타입 정보는 코드가 컴파일되어 자바스크립트로 변환되기 이전에만 존재하기 때문에 그렇습니다. 개발 중에 타입스크립트로 `as`가 포함된 코드를 작성하더라도, 코드가 자바스크립트로 컴파일 될 때는 `as` 키워드를 비롯한 타입스크립트의 타입 정보가 삭제되고, 실제로 런타임에서 실행되는 것은 순수한 자바스크립트 코드 뿐입니다.

[Typescript 공식 문서의 "Erased Types" 항목](https://www.typescriptlang.org/docs/handbook/2/basic-types.html#erased-types)에서도, 이와 관련된 정보를 확인할 수 있습니다.
...
마지막으로 소개드릴 방법은 [Zod](https://github.com/colinhacks/zod) 라이브러리를 사용하는 것입니다. Zod는 타입스크립트를 우선하는 스키마 선언 및 검증 라이브러리입니다. 이 라이브러리를 사용해서 앞서 보았던 여러 방식의 문제점들을 해결할 수 있는데요, 먼저 아래 예시를 보겠습니다.
...
- if문을 통해 Type narrowing을 할 때와는 달리, Zod 스키마는 선언적으로 작성되어 코드를 읽는 사람이 스키마의 구조를 쉽게 파악할 수 있습니다.
...
- `.parse`에서 반환된 데이터는 `as` 키워드를 사용한 Type Assertion과는 다르게, 데이터가 스키마와 일치하는지 실제로 런타임에서 검증되기 때문에, 런타임에도 안심하고 데이터를 다룰 수 있습니다.

또한, 만약 Zod 스키마 대신 타입스크립트 타입을 직접 사용해야 한다면, 이 또한 Zod에서 제공하는 [Type inference](https://github.com/colinhacks/zod#type-inference) 기능을 활용해 Zod 스키마에서 타입스크립트 타입을 쉽게 추출할 수 있습니다. 즉, Zod 스키마 하나만 작성하면 타입스크립트 타입을 중복으로 작성할 필요도 없습니다.
...
Zod가 Joi 대비해서 인기를 얻은 큰 이유 중 하나는 Zod의 훌륭한 타입스크립트 지원입니다. Joi는 데이터에 대한 검증을 해 줄 때, 그 검증으로 얻은 정보가 타입스크립트 타입으로 남지 않습니다. 반면에 Zod에서 데이터를 파싱한 후에는 데이터의 타입을 타입스크립트의 타입으로 확인할 수 있습니다. 때문에, 타입스크립트 생태계 내에서 Zod를 사용하면 (Joi를 사용하는 것과 비교해서) 개발자 생산성을 크게 개선시킬 수 있다고 생각합니다.

Zod와 다른 데이터 검증 라이브러리들 간의 비교는 [Zod 레포지토리의 Comparison 항목](https://github.com/colinhacks/zod#comparison) 에서도 확인해볼 수 있습니다.

## 마치며

위와 같이 외부에서 들어온 데이터를 검증하는 여러가지 방식에 대해서 살펴보았습니다. 저는 Zod를 사용하기 전의 타입스크립트 프로젝트에서는 앞 세 가지 방식을 혼용해서 외부 데이터를 다뤄 왔었는데, Zod를 사용하고 나서는 외부 데이터를 처리하는 것이 이전에 비해 아주 편리하고 안전해진 것을 느끼고 있습니다.
